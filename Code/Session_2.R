...
# load data
data_oc <- read.csv(
  "http://datadryad.org/bitstream/handle/10255/dryad.39576/OstraMRegS400JB.txt?sequence=1",
  sep = "\t"
)
...
step(
  mod_1, direction = "backward", trace = 100,
...
mod_5_s <- lm(
  data_oc2$E100 ~ . - LAT - RC - SA, data = data_env,
  x = TRUE, y = TRUE
)
...
dev.off()
hier.part(
  data_oc2$E100, data_env[, !(names(data_env) %in% "LAT")],
  gof = "Rsqu"
)
