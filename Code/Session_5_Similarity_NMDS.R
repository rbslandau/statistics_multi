...
eu_dist <- vegdist(mat1, method = "eu")
eu_dist
# There is an almost 10 fold spread between highest and lowest distance.
# The species with high abundances dominate the calculation of the distances.
# Therefore, the euclidean distance is often standardised before calculation
# Sites 1 and 3 are very distant as well as sites 2 and 3
# Sites 1 and 5 are closest, 1 and 4 relatively close

# calculation of bray curtis dissimilarity
br_dist <- vegdist(mat1, method = "bray")
br_dist
# Ratio of lowest to highest dissimilarity 4-fold
# Sites 1 and 3 among the more similar
# Sites 1 and 5 most similar
# Sites 1 and 4 relatively dissimilar

### calculation of jaccard index
jac_dist <- vegdist(mat1, method = "jac", binary = T)
jac_dist
# Sites 2 and 3 most similar, and 1 relatively similar to all others
# all other combinations higher dissimilarity

# b)
# standardization of data
# the decostand function contains several standardization methods for community ecologists
mat2 <- decostand(mat1, method = "max")

# Calculation of standardized euclidean distance and comparison to initial results
eu_dist
eu_dist2 <- vegdist(mat2, method = "eu")
eu_dist2
# now only 3-fold spread between highest and lowest distance
# nevertheless, ecologically sites 2 and 3 most similar,
# but this is still not recognized by the euclidean distance

# Calculation of standardized bray curtis dissimilarity and comparison to initial results
br_dist
br_dist2 <- vegdist(mat2, method = "bray")
br_dist2
# scaling decreases most dissimilarities,
# but increases for 1 and 5
# sites 2 and 3 now among the most similar

# Calculation of standardized jaccard dissimilarity and comparison to initial results
jac_dist
jac_dist2 <- vegdist(mat2, method = "jac", binary = T)
jac_dist2
# no effect of standardization!

identical(c(jac_dist), c(jac_dist2))
# test for equality
# c() has to be used as otherwise objects contain original
# call and would differ
